...
| `/controller/debug_path` | nav_msgs/Path | 调试轨迹 |
...
| `/controller/get_diagnostics` | controller_ros/GetDiagnostics | 获取诊断 |
...
# 在 ROS 环境中测试
rostest controller_ros test_controller.test
```
...
├── src/controller_ros/      # Python 模块 (适配器、桥接层等)
└── test/
    ├── test_adapters.py
    └── test_bridge.py
```
